...
async function testURLScrapeWithAuth(token) {
    console.log('\n测试带认证的URL导入功能...');
    ...
    try {
        ...
        if (response.data.success) {
            ...
            console.log('❌ URL导入失败:', response.data.message);
        }
...
async function testShareAPI() {
    ...
    try {
        // 测试分享API，这个应该不需要JWT认证
        const kbId = '8209ef48-c0bd-4e1f-b280-723f3d583fad';
        
        const response = await axios.get('http://localhost:8001/share/v1/app/web/info', {
            headers: {
                'X-KB-ID': kbId
            }
        });
        
        console.log('分享API响应:', response.status);
        console.log('响应数据:', JSON.stringify(response.data, null, 2));
        
        if (response.data.success !== false) {
            console.log('✅ 分享API正常工作');
        } else {
            console.log('❌ 分享API失败:', response.data.message);
        }
        
    } catch (error) {
        console.error('❌ 分享API测试失败:', error.message);
        if (error.response) {
            console.error('错误响应:', error.response.data);
        }
    }
...
async function main() {
    ...
    if (token) {
        await testURLScrapeWithAuth(token);
    }
}
